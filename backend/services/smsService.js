...
/**
 * SMSIndia Hub SMS Service for Create Bharat
...
class SMSIndiaHubService {
  ...
  async sendOTP(phone, otp) {
    try {
      ...
      // Use simple OTP message that doesn't require template approval
      const message = `Your Create Bharat verification code is ${otp}. Valid for 10 minutes. Do not share this code with anyone.`;
      ...
      // Check if response is an object with ErrorCode field (JSON response)
      if (response.data && typeof response.data === 'object' && response.data.ErrorCode) {
        // If ErrorCode exists, it means there's an error
        if (response.data.ErrorCode !== '000' && response.data.ErrorCode !== 'Success') {
          throw new Error(`SMSIndia Hub API error (${response.data.ErrorCode}): ${response.data.ErrorMessage || 'Unknown error'}`);
        }
      }
  ...
  async testConnection() {
    try {
      ...
      const testPhone = '919109992290'; // Use a sample phone number
      const testMessage = 'Your Create Bharat verification code is 12345. Valid for 10 minutes. Do not share this code with anyone.';
